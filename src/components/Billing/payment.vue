<template>
    <div class="payment">
        <v-snackbar v-model="snackbar" :color="color" :timeout="6000" :top="y === 'top'">
            {{ message }}
        </v-snackbar>
        ...
        <v-container class="my-5">
            ...
            <v-layout row justify-right>
                ...
                <v-flex sm12 md6 offset-md2 text-xs-right>
                    <v-toolbar dense floating>
                        <v-text-field hide-details prepend-icon="search" single-line v-model="search" label="Search" v-on:keyup.enter="initialize()"></v-text-field>
                    </v-toolbar>
                </v-flex>
            </v-layout>
            <v-data-table :headers="headers" :items="item" :loading="loader" class="elevation-1">
                <template v-slot:items="props">
                    ...
                    <td class="justify-center layout px-0">
                        <v-btn outline small title="Edit" color="teal" flat router :to="{name:'ShowPayment', params:{id: props.item.id}}">
                            Edit
                            <v-icon right dark>edit</v-icon>
                        </v-btn>
                        <v-btn outline small title="Delete" color="pink" flat @click="deleteItem(props.item)">
                            Delete
            ...
            <div v-if="length" class="text-xs-center">
                <v-pagination :length="length" :total-visible="pagination.visible" v-model="pagination.page" @input="initialize" circle>
                </v-pagination>
            </div>
        </v-container>
        ...
        save() {
            ...
            if (this.editedIndex > -1) {
                ...
                if (this.$refs.productform.validate()) {
                    this.loading = true
                    apiCall({
                            url: '/api/item/' + this.editedItem.id,
