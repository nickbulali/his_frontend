      ...
      <td colspan="4">
        <table>
          <tr>
            ...
            <td>
              Invoice Number: {{ data.invoice.number }}<br> Date: {{ data.invoice.date }}<br> Due Date: {{ data.invoice.due_date }}
            </td>
          </tr>
        </table>
      </td>
    ...
    <tr class="information">
      <td colspan="4">
        <table>
          <tr>
                <td>
              Patient Address: {{ data.invoice.patient.address }}
            </td>
             <td>
              Patient Name: {{data.invoice.patient.name.text }}<br> Patient Identifier: {{ data.invoice.patient.identifier }}<br>Patient Birth Date: {{ data.invoice.patient.birth_date }}<br>Gender: {{  data.invoice.patient.gender_id }}
            </td>
          </tr>
        </table>
      </td>
...
export default {
 ...
 created() {
    ...
    methods: {
      initialize() {
        this.loader=true
        apiCall({ url: "/api/invoice/" + this.$route.params.id, method: "GET" })
          .then(resp => {
