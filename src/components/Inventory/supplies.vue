  ...
  export default {
    ...
    methods: {
      ...
      issueItem(item){
        ...
        apiCall({url: '/api/requestIssue/'+item.item_id, method: 'GET' })
        .then(resp => {
          let request = resp
      ...
      deleteItem (item) {
        ...
        if (this.delete) {
          ...
          apiCall({url: '/api/supplies/'+item.id, method: 'DELETE' })
          .then(resp => {
            console.log(resp)
          })
          .catch(error => {
      ...
      save () {
        ...
        if (this.editedIndex > -1) {
          if(this.$refs.form.validate()){
            this.loadingMethod(true, "Updating Supplies")
            this.loading = true
            apiCall({url: '/api/supplies/'+this.editedItem.id, data: this.editedItem, method: 'PUT' })
            .then(resp => {
        ...
        // store
          ...
          if(this.$refs.form.validate()){
            this.loadingMethod(true, "Adding Supply")
            this.loading = true
            apiCall({url: '/api/supplies', data: this.editedItem, method: 'POST' })
            .then(resp => {
              this.item.push(resp)
              this.resetDialogReferences();
              this.saving = false;
      ...
      saveUpdateStock () {
        if(this.$refs.updateform.validate()){
          this.loading = true
          apiCall({url: '/api/stock/'+this.editedStockItem.id, data: this.editedStockItem, method: 'PUT' })
          .then(resp => {
      ...
      saveIssueStock () {
        if(this.$refs.issueform.validate()){
          this.loading = true
          apiCall({url: '/api/issueStock', data: this.requestItem, method: 'POST' })
            .then(resp => {
              console.log(resp)
