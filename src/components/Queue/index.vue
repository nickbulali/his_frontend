			...
			<div class="his_card_no_shadow">
				<v-layout
					...
					<v-flex sm12 md3>
						...
						<h3 class="title text-xs-left grow mx-3">
							{{ new Date().getUTCDate() }} {{ monthNames[new Date().getMonth()] }} {{ new Date().getFullYear() }}</h3>
					</v-flex>
					<v-flex sm12 md9>
						  ...
						  <v-timeline align-top dense>
							<v-timeline-item color="yellow" small>
								<v-layout pt-1>
										...
										<v-flex xs4>
											<v-btn class="his_card_button white--text"
												small title="Edit" color="green" :loading="loading" round @click="">
			...
			<v-layout
				...
				<v-flex sm12 md12>
					<div class="his_card_no_shadow mx-2">
						...
						<patient-queue v-if="dataready" :dataset="dataset" />
					</div>
				</v-flex>
				<v-flex sm12 md12>
					<v-layout
...
<script>
	...
	export default {
		...
		data () {
				...
				inputRules: [v => v.length >= !v  || 'Field is required'],
				time: '',
				queue:[],
				page: 1,
				vitals:{
